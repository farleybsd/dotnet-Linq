...
namespace Array_Multidimensional
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            //}
            for (int copa = 0; copa <= resultados.GetUpperBound(1); copa++) //dimensao
            {
                int ano = 2014 - copa * 4;

                Console.Write(ano.ToString().PadRight(12)); // tabular
            }

            Console.WriteLine();

            for (int posicao = 0; posicao <= resultados.GetUpperBound(0); posicao++) //dimensao
            {
